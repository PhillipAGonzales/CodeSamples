/* 
	PhillyGsAutoDriver.h for initializing the SparkFunAutoDriver stepper motor driver
	Created by: Phillip A. Gonzales, Dec 5, 2019
	Revised: 12/11/2019 @ 3:39 pm
*/

#ifndef _PHILLYGSAUTODRIVER_h
#define _PHILLYGSAUTODRIVER_h

#include "Arduino.h"
#include "SPI.h"

// Data-Sheet Defines
// dSPIN direction options: functions that accept direction as an argument can be
...
#define STEP_FS_2	0x01   // two uSteps per full step
#define STEP_FS_4	0x02   // four uSteps per full step
...
#define KVAL_ACCEL           0x0B
#define KVAL_DECEL			 0x0C
#define INT_SPD              0x0D
#define ST_SLP               0x0E
...
class PhillyGsAutoDriver
{
	public:
		PhillyGsAutoDriver(int boardPosition, int CSpin, int resetPin, int busyPin);		
		PhillyGsAutoDriver(int boardPosition, int CSpin, int resetPin);
	...
	//General use Functions
		...
		void configSyncPin(byte pinFunction, byte syncSteps);
		void setOverCurrentThreshold(byte threshold);
		void configStepMode(byte stepMode);
		void setAccelKval(byte kvalInput);
		void setDecelKval(byte kvalInput);
		void setRunKval(byte kvalInput);
		void setHoldKval(byte kvalInput);
		void setOsciMode(int oscillatorMode);
		void setOverCurrentShutdown(byte OCshutdown);
		void setPWMfreq(int divisor, int multiplier);
		void setSlewRate(int slewRate);
		void setSwitchMode(int switchMode);
		void setVoltageComp(int vCompMode);
		void setMinSpeed(float stepsPerSec);
		void setMaxSpeed(float stepsPerSec);
		void setFullSpeed(float stepsPerSec);
		void setAccel(float stepsPerSec2);
		void setDecel(float stepsPerSec2);
		...
		long getPos();
		long getMark();
		void run(byte direction, float stepsPerSec);
		void stepClock(byte dir);
		void move(byte direction, unsigned long numSteps);
		void goTo(long pos);
		void goToDir(byte direction, long pos);
		void goUntil(byte action,byte direction, float stepsPerSec);
		void releaseSw(byte action, byte direction);
		void goHome();
		void goMark();
		...
		void softHiZ();
		void hardHiZ();
		void timeRun(byte direction, float speed, float duration, int stopType);
		void stepRun(byte direction, float speed, float numOfSteps, int stopType);
	...
	//General use variables
		...
		byte getRunKval();
		byte getHoldKval();
		byte getOverCurrentThreshold();
		int getOverCurrentShutdown();
		int getPWMfreqDivisor();
		int getPWMfreqMulitplier();
		int getSlewRate();
		int getSwitchMode();
		int getVoltageComp();
		float getMinSpeed();
	...
	private:	
		//Protecting original values of variables
		int _CSpin;
		int _resetPin;
		int _switchPin;
		int _busyPin;
		...
		long xferParameters(unsigned long value, byte bitLength);
		long parameterHandler(byte parameter, unsigned long value);
		
		//Converting human units -> L6470 units
		unsigned long accelCalc(float stepsPerSec2);
		unsigned long decelCalc(float stepsPerSec2);
		unsigned long minSpeedCalc(float stepsPerSec);
		unsigned long maxSpeedCalc(float stepsPerSec);
		unsigned long FSCalc(float stepsPerSec);		//Calculates the FS_SPD register 
		unsigned long intSpeedCalc(float stepsPerSec);
		unsigned long speedCalc(float stepsPerSec);
		
		//Converting L6470 units -> human units
		float accelParse(unsigned long stepsPerSec2);
		float decelParse(unsigned long stepsPerSec2);
		float minSpeedParse(unsigned long stepsPerSec);
		float maxSpeedParse(unsigned long stepsPerSec);
		float intSpeedParse(unsigned long stepsPerSec);
		float FSParse(unsigned long stepsPerSec);
		float speedParse(unsigned long stepsPerSec);
};
#endif